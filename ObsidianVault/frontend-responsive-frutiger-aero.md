...
- **`GlossButton` gotcha (fixed 2026-08-09):** `shape="circle"` used to inherit the size table's horizontal `px` (meant for pill/card text labels) on top of `aspectRatio: 1`, which squeezed an icon-only circle down to a sliver a few pixels wide on web — the `Plus` icon in `SkillsField`'s "add skill" button rendered as a tiny dot instead of a `+`. `gloss-button.tsx` now zeroes `px` and sets `width = height` explicitly whenever `shape === 'circle'`, regardless of `btnSize`. See [[etag-304-body-loss]] for the sibling bug reported alongside this one.
...
- **`onlyAllowShorthands: true`** forces shorthand-only for any style prop that HAS a shorthand — `z` not `zIndex`, `self` not `alignSelf`, `minW` not `minWidth`, `shrink` not `flexShrink`, `pos` is NOT a real shorthand (use `position`, which has none defined and stays available in longhand). This applies inside `$md`-style media-query objects too, not just top-level props — see [[bugfix-lobby-config-flexbasis-mobile-2026-09-16]].
