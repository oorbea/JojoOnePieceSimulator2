...
- ~~Auth/session flow is incomplete — Google login exists, rest of the session/auth lifecycle is WIP.~~ **No longer true, corrected 2026-09-02** — see [[auth-hardening-2026-09-02]]. The flow is complete and verified in prod: Google ID token verification (`aud` pinned to `GOOGLE_CLIENT_ID`), HS256 JWT with alg/issuer/exp validation and a ≥32-char secret enforced at boot, per-IP login rate limit, `RequireAuth`/`RequireAdmin`, admin promotion by `ADMIN_EMAILS`. Three risks were *accepted*, not missing work: ~~JWT in the query string for SSE/WS~~ **closed 2026-09-03**, see [[stream-connection-tickets-2026-09-03]] — replaced by a short-lived, single-use connection ticket ([[picture-events-sse]], [[game-realtime-transport]]); ~~session in `localStorage` on web ([[frontend-stack]]) and no refresh tokens ([[backend-contract]] — 24h JWT, 401 means re-login)~~ both **closed 2026-09-05** — see [[session-token-storage-2026-09-05]]: memory-only access token (15m) + rotating HttpOnly refresh cookie, real server-side logout.
...
- **2026-09-03**: any logged-in user (not just `ADMIN`) can now browse Stands/Devil Fruits/Stages at `/catalog/*`, read-only — the admin screens' write surface was already isolated into two prop bundles, so the read-only variant is a `readOnly` discriminated union on the same screens, not a fork. New shared `DetailModal` ("card in the foreground") reused by both admin and catalogue. Home's channel grid is now fully unlocked (Stages replaces the placeholder "Powers" slot) and the top nav's active-tab highlight is a sliding Reanimated pill instead of a flat background swap. See [[catalogo-publico-stands-devil-fruits-stages]] and [[nav-indicador-deslizante]].

- **2026-09-05**: nginx now sends a real Content-Security-Policy plus X-Content-Type-Options/Referrer-Policy/Permissions-Policy (previously only COOP), and the backend keys rate limits on the real client IP behind Nginx Proxy Manager (`router.go` chains `ClientIPFromXFF()` after `ClientIPFromRemoteAddr`) instead of NPM's own IP for every caller. Closes gaps flagged in [[session-token-storage-2026-09-05]] and [[stream-connection-tickets-2026-09-03]]. See [[csp-y-rate-limit-por-ip-2026-09-05]].
- **2026-09-17**: the lobby's share button now shares a join link (`https://<origin>/join/<token>`), not the raw code — short-lived (15m), multi-use `IGameInviteStore` (memory/Redis, copied from `streamticket`'s shape but never-consumed `Lookup` instead of `Redeem`). Revocation on join-code rotation is a comparison against the game's *current* code at redeem time, not a second write inside `RegenerateGameCode`'s lock — chosen specifically because the rejected alternative (a per-game token set, `DEL`'d on rotation) fails open on a Redis error, the opposite of what revocation is for. Public `GET /games/invite/{token}/status` answers only VALID/EXPIRED, nothing else, so an unauthenticated caller (including a chat app's unfurl bot) learns nothing about the lobby. No ban/kick list was added — an invite link can't distinguish "kicked" from "code rotated", both just read as expired, matching how a join code already worked. See [[game-invite-links-2026-09-17]].

- **2026-09-17 (fix, unrelated to the above)**: found during that feature's live two-account walkthrough — `deployments/.env.example`'s `CORS_ALLOWED_HEADERS` never included `X-JOPS-Refresh` (the CSRF header `/auth/refresh`/`/auth/logout` require, added in [[session-token-storage-2026-09-05]]), even though the Go default (`config.go`) already did. Any local `.env` copied from the example silently broke silent-refresh in the one place cross-origin CORS actually applies here — frontend and backend on different local ports — while prod never noticed since it's same-origin behind Nginx Proxy Manager. Symptom: a hard page reload logs the user straight to `/login` with a browser-reported `503` on the `POST /auth/refresh` that never reaches the Go process (the preflight silently fails to grant the header, so the browser never sends the real request). Fixed by adding `X-JOPS-Refresh` to the example's default.
