...
- [[entrega-imagenes-red-lenta-2026-09-07]] / [[media-proxy-content-addressed]] — slow-network catalogue image fix (2026-09-07/08): T1 (card rendition + LQIP + `/stands/options` + gzip) shipped; T2 (immutable content-addressed media proxy, `GET /api/v1/media/...`) shipped 2026-09-08, backfill (`cmd/mediabackfill`) not run yet. T3 partially shipped 2026-09-08: `image-queue.ts` + `LazyImage`/`Skeleton` fix the actual concurrency-stampede bug, adopted so far only in `stand-card.tsx`; backend pagination and the remaining catalogue-card adoptions are still open.
