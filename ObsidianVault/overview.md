...
- [[entrega-imagenes-red-lenta-2026-09-07]] / [[media-proxy-content-addressed]] / [[catalogue-pagination]] — slow-network catalogue image fix (2026-09-07/08): T1 (card rendition + LQIP + `/stands/options` + gzip) shipped; T2 (immutable content-addressed media proxy, `GET /api/v1/media/...`) shipped 2026-09-08, backfill (`cmd/mediabackfill`) not run yet. T3 partially shipped 2026-09-08: `image-queue.ts` + `LazyImage`/`Skeleton` fix the actual concurrency-stampede bug (adopted so far only in `stand-card.tsx`), keyset pagination (`?limit=`/`?cursor=`, opt-in) is live for all three of `GET /stands`/`GET /devil-fruits`/`GET /stages`, and the frontend `usePaginatedCatalogue` hook + "Cargar más" UI is wired into the public Stand catalogue as the reference adoption. DevilFruit/Stage catalogue UI adoption and the remaining `LazyImage` card adoptions are still open.
