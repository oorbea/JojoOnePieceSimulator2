...
func newVersusLobby(t *testing.T, teamSize int) (*game.Game, *game.Participant, *game.Participant, *game.Team, *game.Team) {
	t.Helper()
	cfg, err := game.NewConfig(enums.Versus, []enums.Manga{enums.Jojo}, enums.Random, teamSize, false, enums.Private, 30, game.PoolFilter{})
	if err != nil {
...
func TestReconfigure_HostOnlyAndLobbyOnly(t *testing.T) {
	...
	next, err := game.NewConfig(enums.Versus, []enums.Manga{enums.Jojo}, enums.Random, 2, false, enums.Public, 45, game.PoolFilter{})
	if err != nil {
		t.Fatalf("NewConfig: %v", err)
	}
...
func TestReconfigure_GauntletToVersusSplitsPlayers(t *testing.T) {
	...
	next, err := game.NewConfig(enums.Versus, []enums.Manga{enums.Jojo}, enums.Random, 2, false, enums.Private, 30, game.PoolFilter{})
	if err != nil {
		t.Fatalf("NewConfig: %v", err)
	}
...
func TestReconfigure_VersusToGauntletMergesPlayers(t *testing.T) {
	g, host, other, _, _ := newVersusLobby(t, 2)
	next, err := game.NewConfig(enums.Gauntlet, []enums.Manga{enums.Jojo}, enums.Random, 10, false, enums.Private, 30, game.PoolFilter{})
	if err != nil {
		t.Fatalf("NewConfig: %v", err)
	}
...
func TestReconfigure_ShrinkingBelowSeatedHumansIsRejected(t *testing.T) {
	...
	shrunk, err := game.NewConfig(enums.Versus, []enums.Manga{enums.Jojo}, enums.Random, 1, false, enums.Private, 30, game.PoolFilter{})
	if err != nil {
		t.Fatalf("NewConfig: %v", err)
	}
	before := g.Config()
