...
// Reaper periodically removes expired invites from a MemoryStore so a
// burst of minted-but-never-redeemed (or simply outlived) invites doesn't
// sit in the map until process exit - mirrors streamticket.Reaper's
// Start/ReapOnce split. Only meaningful for MemoryStore: the Redis store
...
func (r *Reaper) Start(ctx context.Context) {
	...
	for {
		...
		case <-ticker.C:
			if n := r.ReapOnce(); n > 0 {
				log.Printf("game invite reaper: removed %d expired invite(s)", n)
			}
		}
	}
}

// ReapOnce removes every expired invite, returning how many were removed.
