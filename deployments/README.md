...
| `MEDIA_BASE_URL` | `[CONFIG]` | Origin+prefix media URLs are built under. Prod: relative (`/api/v1/media`, same-origin behind NPM). **Local dev must be absolute, pointed at the frontend's own origin** (`http://localhost:3000/api/v1/media`, not the backend's `:8080`) — a relative URL falls through to the frontend nginx's SPA fallback with a silent 200/`index.html` instead of an image, and pointing straight at `:8080` is cross-origin, which the service worker's media cache and the CSP `img-src 'self'` rule both silently refuse. `nginx.frontend.conf.template`'s `/api/v1/media/` location proxies this back to the backend so `:3000` stays same-origin end to end — see `ObsidianVault/sw-cache-media-scope-privado-2026-09-09.md`. |
